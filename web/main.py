...
# 網頁版：瀏覽器冇本地檔案系統，排行榜寫入改為丟棄（避免遊戲結束時 crash）
import io
_real_open = open
def open(path, mode='r', *args, **kwargs):
    if 'leaderboard.json' in str(path) and any(c in mode for c in 'wa+'):
        return io.StringIO()
...
def load_leaderboard():
   if os.path.exists("leaderboard.json"):
       with open("leaderboard.json", "r") as f:
           return json.load(f)   # 轉換成 Python 數據
   return []   # 檔案不存在就回傳空陣列
...
def start_panel(game_state, player_name, running):
   screen.fill((0, 0, 0))
   screen.blit(qr_img, (10, 10))   # 在開始畫面顯示 QR code
...
# 主迴圈
async def main():
   global running, game_state, player_name, snake_head, snake_body, apple_x, apple_y, direction, change_to, score, scored_saved
   while running:
      if game_state == "start":
      ...
      if game_state == "play":
          ...
          # 吃到食物（距離一格內都算，hitbox 較大）
          if abs(snake_head[0] - apple_x) <= 10 and abs(snake_head[1] - apple_y) <= 10:
              score += 10
              point_sound.play()
              apple_x, apple_y = random.randrange(0, 710, 10), random.randrange(0, 470, 10)
          else:
              snake_body.pop()   # 移除陣列尾元素
